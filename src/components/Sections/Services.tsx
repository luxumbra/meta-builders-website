    ...
    <ContentSection title="Services" id={sectionId} lead={<ServicesLead />}>
      <ul className="grid max-w-6xl grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {services.map(({ title, description, icon }) => (
          <li key={uuid()} className="flex flex-col items-center gap-2 2xl:gap-4 p-6 2xl:p-6 border border-default bg-glass-secondary-200 dark:bg-glass-primary-900 backdrop-blur-2xl">
            ...
            <h3 className="text-base 2xl:text-xl font-extrabold text-center text-teal-300">{title}</h3>
