...
export function Layout({ children, content }: {children: React.ReactNode, content: LayoutProperties}): JSX.Element {
  ...
  return (
    <div className="wrapper h-full w-full bg-gradient-to-b dark:from-slate-900 dark:to-slate-800 overflow-x-hidden">
      ...
      <Header />
      {children}
      <HashLink to="/#home" className="group inline-flex invisible items-center back-to-top py-3 text-sm 2xl:text-lg text-normal uppercase font-sans fixed bottom-12 right-2 2xl:bottom-24 2xl:right-16 z-40"><span className="gradient-text text-shadow-alt-md-teal ">Top </span> <Icon icon="emojione-monotone:index-pointing-up" className='h-5 w-5 2xl:h-8 2xl:w-8 text-violet-500 inline '/></HashLink>
      <Footer />
    </div>
  );
}
